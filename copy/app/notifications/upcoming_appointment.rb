...
#
# UpcomingAppointment.with(post: @post).deliver_later(current_user)
# UpcomingAppointment.with(post: @post).deliver(current_user)

class UpcomingAppointment < ApplicationNotification
  ...
  deliver_by :email, mailer: "AppointmentMailer", method: :upcoming_appointment_email
  ...
  def message
    t("notifications.created_appointment.message", appointment_date: appointment.date.strftime("%b %e, %Y"), link: url)
  end

  def subject
    t("notifications.created_appointment.subject", appointment_date: appointment.date.strftime("%b %e, %Y"))
  end
