class AccountsController < Accounts::BaseController
  ...
  before_action :authorize_user!, only: [:update, :create, :edit]

  # GET /accounts
  def index
    @pagy, @accounts = pagy(Account.impersonal.where(active: true))
    @pagy, @accounts = pagy(Account.impersonal) if current_user.admin?
    authorize! @accounts
  end
