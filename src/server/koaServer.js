...
const openUrls = [ // urls that don't require the user to be logged in
    ...
    '/api/register',
    '/api/sessionStatus',
     '/api/completePromotion',
     '/api/canvassAppointment',
     '/api/canvassInteraction',
    '/api/roles',
...
async function authMiddleware(ctx, next) {
    if (!openUrls.some(url => ctx.request.url.indexOf(url) === 0)) {
        const { isActive } = await sessionService.getSession(ctx.cookies.get(constants.SESSION_KEY_COOKIE_NAME))
        if (!isActive) {
            ctx.status = 401
            return
        }
