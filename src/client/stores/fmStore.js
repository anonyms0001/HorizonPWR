...
export default store({
    ...
    eventListeners: {
        LoginConfirmed({ entities }) {
            const { statsFM } = entities
            emit.ReceivedStatsFM({ statsFM })
        ...
        async RouteChanged({ pathname }) {
            ...
            if (pathname === '/leaderboard' || pathname === '/repsLeaderboard') {
                ...
                // if (this.energyConsultants === null) {
                    ...
                    // date = date.toIsoString()
                    // date = date.split("T")
                    // console.log("fm store begining date ", date[0])
        ...
        ReceivedStatsFM({ statsFM }) {
            // console.log("team stats store ",   statsFM.FieldMarketersFM)
            this.fmStats = statsFM.FieldMarketersFM
