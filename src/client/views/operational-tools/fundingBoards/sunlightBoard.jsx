...
@view
export default class SunlightBoard extends React.Component {

    render() {
        const {selectedFundingCard, fundingBoardAlert, proposalFundingReloading, financialPartnersFunding, sunlightExpanded} = fundingStore
        const {user} = sessionStore

        let fundingPayments = financialPartnersFunding.Sunlight
        ...
        for (let i = 0; i < fundingPayments.length; i++) {
            console.log(i, fundingPayments[i].Name)


            if (fundingPayments[i].Pending_Cancellation__c || fundingPayments[i].Project_Cancelled__c ) {
                continue;
            }
                        ...
                        tags: [
                            {
                                ...
                                title: "Log First Funding Payment",
        ...
        function expandToogle() {
            if (sunlightExpanded) {
                emit.ClickedExpandedBtn({financePartner: 'sunlight', selectedState: false})
            } else {
                emit.ClickedExpandedBtn({financePartner: 'sunlight', selectedState: true})
            }
        }
                ...
                <div className="panel  panel-default" style={{margin: "0px", boxShadow: "none"}}>
                    <div className="panel-heading">
                        <div style={{display: "flex"}}>
                            <div style={{flex: "1", textAlign: "left", fontSize: "1.3em"}}>Sunlight - Needs First
                                Payment: {firstColLabel} - Needs Second Payment: {secondColLabel}
                            </div>
                            <button className="btn btn-default expand-btn" style={{border: "none"}}
                                    onClick={expandToogle.bind(this)}>
                                {sunlightExpanded ?
                    ...
                    <div id='funding-body-panel' className="panel-body"
                         style={sunlightExpanded ? {display: "block", padding: "0px"} : {display: "none"}}>
