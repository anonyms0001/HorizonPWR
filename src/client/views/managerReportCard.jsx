...
export default view(function ManagerReportCard() {
    ...
    while (++f < fieldMarketers.length) {
        ...
        if (fieldMarketers[f].Team === user.teamName) {
            teamFMCount += 1
            totalTeamQS += fieldMarketers[f].Qs
            totalTeamLeads += fieldMarketers[f].Leads
            totalTeamAssists += fieldMarketers[f].Qs
    ...
    let teamECCount = 0
    let totalTeamSits = 0
    let totalTeamCloses = 0
    let totalTeamSheduledInstalls = 0


    while (++e < energyConsultants.length) {

        if (energyConsultants[e].Team === user.teamName) {
            teamECCount += 1
            totalTeamSits += energyConsultants[e].Sits
            totalTeamCloses += energyConsultants[e].Closes
            totalTeamSheduledInstalls += energyConsultants[e].ScheduledInstalls
        }
    }
    ...
    //Team Calculations
    let fmTeamLeadUnit = ((((totalTeamLeads / teamFMCount) / 8) * .8) * 100).toFixed(2)
    let fmTeamQsUnit = ((((totalTeamQS / teamFMCount) / 6) * .8) * 100).toFixed(2)
    let fmTeamAssistsUnit = ((((totalTeamAssists / teamFMCount) / 1.5) * 1.3) * 100).toFixed(2)
    let teamSitsUnit = ((((totalTeamSits / teamECCount) / 8) * .9) * 100).toFixed(2)
    let teamClosesUnit = ((((totalTeamCloses / teamECCount) / 1.5) * .9) * 100).toFixed(2)
    let teamISUnit = ((((totalTeamSheduledInstalls / teamECCount) / 1.5) * .9) * 100).toFixed(2)
    let fmTeamUnit = ((parseInt(fmTeamLeadUnit) + parseInt(fmTeamQsUnit) + parseInt(fmTeamAssistsUnit)) / 3).toFixed(2)
    let ecTeamUnit = ((parseInt(teamSitsUnit) + parseInt(teamClosesUnit) + parseInt(teamISUnit)) / 3).toFixed(2)
        ...
        <div className="panel  panel-default">
            <div className="panel-heading">
                <div style={{display: "flex"}}>
                    <div style={{flex: "1", textAlign: "left"}}>
                        <h4>{user.teamName} Report Card </h4>
                    </div>
                    {
            ...
            <div id="chart" className="panel-body">
                <div className="row text-center">
                    ...
                    <div className="col-xs-4">
                        <h1 className="overall-scores text-center" >
                            <b>{((parseInt(fmTeamUnit) + parseInt(ecTeamUnit)) / 2).toFixed(2)}%</b>
                        </h1>
                ...
                {/*            FMs - {teamFMCount} <br/>*/}
                {/*            ECs - {teamECCount} <br/>*/}
                {/*            Total Team Leads: {totalTeamLeads}<br/>*/}
                {/*            Total Team QS: {totalTeamQS}<br/>*/}
                {/*            Total Team Sits: {totalTeamSits}<br/>*/}
                {/*            Total Team Assists: {totalTeamAssists}<br/>*/}
                {/*            Total Team Closes: {totalTeamCloses}<br/>*/}
                {/*            Total Team Installs Scheduled Installs: {totalTeamSheduledInstalls}*/}
                {/*            <br/>*/}
                {/*            <br/>*/}
                {/*            Unit Breakdown<br/>*/}
                {/*            Leads: {fmTeamLeadUnit}%<br/>*/}
                {/*            Qs: {fmTeamQsUnit}%<br/>*/}
                {/*            Sits: {teamSitsUnit}%<br/>*/}
                {/*            Assists: {fmTeamAssistsUnit}%<br/>*/}
                {/*            Closes: {teamClosesUnit}% <br/>*/}
                {/*            Installs: {teamISUnit}%<br/>*/}
                {/*            FM Grade: {fmTeamUnit}%<br/>*/}
                {/*            EC Grade: {ecTeamUnit}% <br/>*/}
                {/*            Team Weighted*/}
                {/*            Score: {((parseInt(fmTeamUnit) + parseInt(ecTeamUnit)) / 2).toFixed(2)} % <br/>*/}
                {/*            <br/>*/}
                {/*            Lead PRA: {(totalTeamLeads / teamFMCount).toFixed(2)}*/}
                {/*            <br/>*/}
                {/*            QS PRA: {(totalTeamQS / teamFMCount).toFixed(2)}*/}
                {/*            <br/>*/}
                {/*            Sits PRA: {(totalTeamSits / teamECCount).toFixed(2)}*/}
                {/*            <br/>*/}
                {/*            Assists PRA: {(totalTeamAssists / teamFMCount).toFixed(2)}*/}
                {/*            <br/>*/}
                {/*            Closes PRA: {(totalTeamCloses / teamECCount)}<br/>*/}
                {/*            Installs Scheduled PRA: {(totalTeamSheduledInstalls / teamECCount)} <br/>*/}
                {/*            <br/>*/}
                {/*            <br/>*/}
                {/*        </h5>*/}
                {/*    </div>*/}
                ...
                <hr/>
                <div className="row">
                    <div className="col-md-6 fms-divider"  >
                        <h3 className="text-center">{user.teamName} FMs - {teamFMCount} </h3>
                        <hr />
                        <div className="row text-center">
                            <div className="col-xs-4">
                                <h2><b>{(totalTeamLeads / teamFMCount).toFixed(2)}</b></h2>
                                <span style={{fontSize: "12px"}}>Leads PRA</span>
                            </div>
                            <div className="col-xs-4">
                                <h2><b>{(totalTeamQS / teamFMCount).toFixed(2)}</b></h2>
                                <span style={{fontSize: "12px"}}>QS PRA</span>
                            </div>
                            <div className="col-xs-4">
                                <h2><b>{(totalTeamAssists / teamFMCount).toFixed(2)}</b></h2>
                                <span style={{fontSize: "12px"}}>Sits PRA</span>
                            </div>
                        </div>
                        <hr />
                        {
                            ...
                            fieldMarketers.length !== 0 ? fieldMarketers.filter(fieldMarketers => fieldMarketers.Team === user.teamName).map((fm) => {
                                return (
                                    <div className="row info-wrapper">
                                        <h3 className="text-center">{fm.Name}</h3>
                                        ...
                                        <div className="col-md-6">
                                            <div style={{display: "flex"}}>
                                                ...
                                                <div style={{flex: "1", textAlign: "center"}}>
                                                    ...
                                                    <h5 style={{color: "rgb(231, 145, 54)"}}><b>{fm.AssistedCloses}</b>
                                                    </h5>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div className="col-md-6">
                        <h3 className="text-center">{user.teamName} ECs - {teamECCount} </h3>
                        <hr />
                        <div className="row text-center">
                            <div className="col-xs-4">
                                <h2><b>{(totalTeamSits / teamECCount).toFixed(2)}</b></h2>
                                <span style={{fontSize: "12px"}}>Sits PRA</span>
                            </div>
                            <div className="col-xs-4">
                                <h2><b>{(totalTeamCloses / teamECCount).toFixed(2)}</b></h2>
                                <span style={{fontSize: "12px"}}>Closes PRA</span>
                            </div>
                            <div className="col-xs-4">
                                <h2><b> {(totalTeamAssists / teamFMCount).toFixed(2)}</b></h2>
                                <span style={{fontSize: "12px"}}>Installs Scheduled PRA</span>
                            </div>
                        </div>
                        <hr />
                        {
                            // initOtherTables === 'fm' && sortedTableFM.length !== 0 ? sortedTableFM.map((stat1) => {
                            // fmStatsSort !== null ? fmStatsSort.map((stat1) => {
                            energyConsultants.length !== 0 ? energyConsultants.filter(energyConsultants => energyConsultants.Team === user.teamName).map((ec) => {
                                return (

                                    <div className="row info-wrapper">
                                        <h3 className="text-center">{ec.Name}</h3>
