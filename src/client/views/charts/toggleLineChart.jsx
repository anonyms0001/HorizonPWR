...
export default view(function ToggleLineChart() {


    const {topLabel, bottomLabel, pointsArray, weeksQuarters} = weeklyReportStore

    let weeksScores = []
    let pointsBackground = []
    for (let i = 0; i < bottomLabel.length; i++) {
        // console.log("weeksScores up loop ", lineChartWeekScores[i])
        weeksScores.push(bottomLabel[i])
        if (bottomLabel[i] < 35) {
            pointsBackground.push('red')
        } else if (bottomLabel[i] > 75) {
    ...
    this.selectionIndex = function (dateRange, positionAtEvent) {
        ...
        emit.ProcessNewPointPosition({position: positionAtEvent, length: (bottomLabel.length - 1)})
