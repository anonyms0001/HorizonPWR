...
class BlockedProposal < ApplicationNotification
  ...
  def message
    t("notifications.blocked_proposal.message", contact_name: (contact&.name ||= "someone"), blocked_on: proposal.blocked_on, appointment_date: proposal.appointment.start_at.strftime("%b %e, %Y, %l:%M %P"), link: url)
  end

  def subject
    t("notifications.blocked_proposal.subject", contact_name: (contact&.name ||= "Someone"), appointment_date: proposal.appointment.start_at.strftime("%b %e, %Y, %l:%M %P"))
  end

  def proposal
    Proposal.find(params[:proposal].id) # TODO: This param is not always passed; we need to find a way to get the proposal without the param
  end
