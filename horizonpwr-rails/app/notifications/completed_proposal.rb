...
class CompletedProposal < ApplicationNotification
  ...
  # TODO: Turn this back on when Proposals function properly.
  ...
  def message
    t("notifications.completed_proposal.message", contact_name: contact_name, appointment_date: proposal.appointment.start_at&.strftime("%B %e, %Y"), link: url)
  end

  def subject
    t("notifications.completed_proposal.subject", contact_name: contact_name, appointment_date: proposal.appointment.start_at&.strftime("%B %e, %Y"))
  end
