class FormConfigsController < ApplicationController
  ...
  def index
    ...
    authorize! @form_configs, with: FormConfigPolicy
    @form_configs.load
  end
  ...
  def new
    ...
    authorize! @form_config, with: FormConfigPolicy
  end
  ...
  def create
    ...
    if @form_config.save
      redirect_to @form_config.dynamic_form, notice: "Form config was successfully created."
    else
  ...
  def form_config_params
    params.require(:form_config).permit(:title, :active, :position, :dynamic_form_id)
  end
end
