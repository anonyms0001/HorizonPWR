class EarningRatesController < ApplicationController
  ...
  before_action :set_earning_type_id, only: [:new, :edit, :update]
  ...
  def create
    ...
    if @earning_rate.save
      redirect_to @earning_type, notice: "Earning rate was successfully created."
    else
  ...
  def set_earning_type_id
    @earning_type = if @earning_rate&.earning_type.present?
      EarningType.find(@earning_rate.earning_type.id)
    else
      EarningType.find(params[:earning_rate][:earning_type_id])
    end
  end
