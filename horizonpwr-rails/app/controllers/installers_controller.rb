class InstallersController < ApplicationController
  ...
  # GET /installers
  def index
    installers = Installer.sorted
    installers = Installer.sort_by_params(params[:sort], sort_direction) if params[:sort].present?
    installers = Installer.search_by_params(params[:q]) if params[:q].present?
    @pagy, @installers = pagy(installers)
  ...
  # GET /installers/1
  def show
    @install_teams = Account.installers
  end
