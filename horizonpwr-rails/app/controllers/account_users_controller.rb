class AccountUsersController < Accounts::BaseController
  ...
  # DELETE /accounts/1/members/1715
  def destroy
    if @account_user.destroy
      redirect_to @account, notice: "User was removed from #{@account.name} team"
    else
      render :edit, status: :unprocessable_entity
    end
  end
  ...
  def deactivate
    @account_user = @account.account_users.find(params[:account_user_id])

    if @account_user.user.deactivate_user!
      redirect_to @account, notice: "DeActivation request for #{@account_user.user.name} sent. HR has been notified"
    else
      redirect_to request.referrer, notice: "DeActivation request Error"
    end
  end
