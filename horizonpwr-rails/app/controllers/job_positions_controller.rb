class JobPositionsController < ApplicationController
  ...
  def index
    ...
    # sort_direction = params[:sort].nil? ? "asc" : params[:direction]
    # @pagy, @job_positions = pagy(JobPosition.sort_by_params(sort_column, sort_direction))
    # @pagy, @job_positions = pagy(JobPosition.search_by_params(params[:q])) if params[:q].present?
  ...
  def show
    ...
    @earning_types = @job_position.earning_types.order(display_name: :asc)
    @earning_type_names = @earning_types.pluck(:display_name, :name).uniq
  end
  ...
  def job_position_params
    params.require(:job_position).permit(:name, :active, :description, :hiring, :hiring_public, :department_id)
  end
end
